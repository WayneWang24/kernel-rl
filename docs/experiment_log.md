...
### 状态
- [x] 训练停止于 step 1200/~2400 (50%)
- [x] KernelBench 编译+运行评测 (2026-03-25)

### KernelBench 编译+运行评测结果 (2026-03-25)
...
| 模型 | L1 编译 | L1 验证 | L2 编译 | L2 验证 |
|------|---------|---------|---------|---------|
| SFT 7B (Exp-1) | 86% | 0% | 81% | 23.5% |
| **GRPO 3B (Exp-2)** | **99%** | 0% | **99%** | 0% |

**关键发现：**
- RL (GRPO) 即使用 3B 小模型也大幅提升编译率 (99% vs 86%/81%)，验证了 RL 方法有效性
- 静态分析 reward 只教会代码结构，无法提升运行正确性 → Exp-3 需要 compile+run reward
- 3B 模型验证率 0% (vs SFT 7B L2 的 23.5%)：小模型缺乏算法理解能力
